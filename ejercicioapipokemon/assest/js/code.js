...
let Nbuscador = document.querySelector(`#Nbuscador`)
let Bbuscador = document.querySelector(`#Bbuscador`)
let Ipokemon = document.querySelector (`#IPokemon`)
...
btnP.addEventListener("click", (e) => {
    url_api = e.target.getAttribute("data-paginaAnterior")
    if (url_api != "null") {
...
btnN.addEventListener("click", (e) => {
    console.log(e)
    url_api = e.target.getAttribute("data-paginaSiguiente")
    if (url_api != "null") {
        obtenerPokemones(url_api)
    }
})
...
async function obtenerPokemones(url_api = 'https://pokeapi.co/api/v2/pokemon') {
    ...
    if(infoPokemon.previous != null && infoPokemon.next != null){
        ...
        btnN.setAttribute("class","d-block")
        btnP.setAttribute("class","d-block")
        ...
        btnP.setAttribute("class","d-none")
        btnN.setAttribute("class","d-block")
    }else if(infoPokemon.previous != null && infoPokemon.next == null){
        contenedorBotones.classList.remove("justify-content-between");
        contenedorBotones.classList.remove("justify-content-end");
        contenedorBotones.classList.add("justify-content-start");
        btnN.setAttribute("class","d-none")
    }
...
Bbuscador.addEventListener("click",async ()=>{
    divGrillaPagina.classList.add("d-flex","justify-content-center")
    btnN.setAttribute("class","d-none")
    btnP.setAttribute("class","d-none")
    divGrillaPagina.innerHTML = ""

    let info = await fetch(`https://pokeapi.co/api/v2/pokemon/${Nbuscador.value}`)
    let pokemonInformacion = await info.json()
    let liTipos = []
    pokemonInformacion.types.forEach(dataTipo => {
        liTipos += `<li>${dataTipo.type.name}</li>`
...
Ipokemon.addEventListener("click",()=>{
    location.reload()
})
